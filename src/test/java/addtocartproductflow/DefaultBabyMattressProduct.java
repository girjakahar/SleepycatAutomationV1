...
public class DefaultBabyMattressProduct extends BaseSleepycat {
	...
	public static Logger log =LogManager.getLogger(DefaultBabyMattressProduct.class);
		...
		public void defaultBabyMattressAddToCart() throws Exception
		{
		   ...
		   log.error("Clicked on add to cart button");
			...
			if(productname) 
			{
				...
				log.error("Single category Baby Mattress Product is added in cart");
			}else
			{
				System.out.println("Single category Baby Mattress Product is added in cart");
				log.error("Single category Baby Mattress Product is not added in cart");
			}	
		}
