...
public class PlusMattressCustomCategory extends BaseSleepycat {
	
	static RemoteWebDriver driver;
	static WebDriverWait wait;
	public static Logger log =LogManager.getLogger(PlusMattressCustomCategory.class);
    ...
    @Test
	public void plusMattessCustomAddToCart() throws Exception
	{
	   ...
	   landingPage.offerModal();
  	   boolean lengthvalidation =productDetails.lenghtError().isDisplayed();
  	   if(lengthvalidation) 
  	   {  
	   ...
	   landingPage.offerModal();
	   WebElement text = driver.findElement(By.xpath("//span[text()='Frequently bought with the above product']"));
	   Actions Scrolltotext = new Actions(driver);
	   Scrolltotext.moveToElement(text).build().perform();
	   log.error("Move to Frequently bought text");
	   ...
	   log.error("Clicked on add to cart button");
  	   ...
  	   landingPage.offerModal();
	    wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[@class='cart_item_mid']//div[@data-product_sku='SC-PLUS-Q-75x60x8']")));
		
		boolean productname = driver.findElement(By.xpath("//div[@class='cart_item_mid']//div[@data-product_sku='SC-PLUS-Q-75x60x8']")).isDisplayed();
		if(productname) 
		{
			System.out.println("Custom Category Plus Mattress Product is added in cart");
			log.error("Custom Category Plus Mattress Product is added in cart");
		}else
		{
			System.out.println("Custom Category Plus Mattress Product is not added in cart");
			log.error("Custom Category Plus Mattress Product is not added in cart");
		}	
